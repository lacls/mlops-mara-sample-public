...
def main(args):
    ...
    df = pd.read_parquet(args.data).sort_values(by="created")
    records = df[["attack_id","created", "feature1", "feature2","label"]].to_dict("records")
    ...
    while True:
        for record in records:
            record["created"] = (
                record["created"] + pd.Timedelta(weeks=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--topic_name', default="network",
                        help='Kafka topic name')
    parser.add_argument('-d', '--data', default="data/network_attack.parquet",
